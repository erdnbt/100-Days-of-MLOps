...
### 2. Ok, we need to update packages
...
but the question: how to check UV requirements for each package.

> every package carries a version constraint that `uv` can actually satisfy against PyPI.

### 3. checking `uv` version

```bash
root@controlplane ~/code ➜  uv -V
...
### 4. Update back to requirements.in file
